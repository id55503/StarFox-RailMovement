    ...
    protected override void goToTarget()
    {
        m_direction = (m_target.position + m_targetOffset - transform.position).normalized *
                      m_distanceInfluence.Evaluate(1 -
                                                   (m_target.position + m_targetOffset - transform.position).magnitude /
                                                   m_searchRange);
        ...
        if (enabled)
        {
            ...
            if (m_drawSearchZone)
            {
                ...
                Handles.DrawSolidArc(transform.position, Quaternion.AngleAxis(90, -transform.right) * transform.forward,
                    Quaternion.AngleAxis(-m_searchAngle / 2, transform.up) * transform.forward, m_searchAngle,
                    m_searchRange);
                Handles.DrawSolidArc(transform.position, transform.forward,
                    Quaternion.AngleAxis(-m_searchAngle / 2, transform.up) * transform.forward, 360, m_searchRange);
            }
